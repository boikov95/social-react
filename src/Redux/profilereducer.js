...
export const saveProfile = (profile) =>
    async (dispatch, getState) => {
        ...
        if (data.resultCode === 0) {
            ...
            data.messages.map((k)=> 
                {
                let key=k.substring(k.indexOf(">")+1, k.length-1).toLowerCase(); 
                arrayEdit.contacts.push({[key]: k});  
                }
            
            );
            let obj = {};
            arrayEdit.contacts.forEach((el)=>{
                Object.assign(obj,el);
            });            
            dispatch(stopSubmit("editprofile", {"contacts": obj}));
        }
    }
